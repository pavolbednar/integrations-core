...
def test_deadlock_failed_obfuscation(instance_docker):
    check = SQLServer(CHECK_NAME, {}, [instance_docker])
    test_xml = """
    <event name="xml_deadlock_report" package="sqlserver" timestamp="2024-08-20T08:30:35.762Z">
     <data name="xml_report">
    ...
    expected_xml_string = (
        ...
        "<process id=\"process12108eb088\"> "
        "<executionStack> "
        "<frame procname=\"adhoc\" line=\"1\" stmtstart=\"38\" stmtend=\"180\" sqlhandle=\"0\">ERROR: "
        "failed to obfuscate</frame> "
        "<frame procname=\"adhoc\" line=\"1\" stmtend=\"128\" sqlhandle=\"0\">ERROR: failed to obfuscate</frame> "
        "</executionStack> "
        "<inputbuf>ERROR: failed to obfuscate</inputbuf> "
        "</process> "
        "<process id=\"process1215b77088\"> "
        "<executionStack> "
        "<frame procname=\"adhoc\" line=\"1\" stmtstart=\"38\" stmtend=\"180\" sqlhandle=\"0\">ERROR: "
        "failed to obfuscate</frame> "
        "<frame procname=\"adhoc\" line=\"1\" stmtend=\"126\" sqlhandle=\"0\">ERROR: failed to obfuscate</frame> "
        "</executionStack> "
        "<inputbuf>ERROR: failed to obfuscate</inputbuf> "
        "</process> "
        "</process-list> "
    ...
    with mock.patch(
        'datadog_checks.base.utils.db.utils.obfuscate_sql_with_metadata', side_effect=Exception("couldn't obfuscate")
    ):
        config = type(
            "Config",
            (object,),
            {"deadlocks_config": {"max_deadlocks": 5}, "log_unobfuscated_queries": True, "obfuscator_options": None},
        )
        deadlocks = Deadlocks(check, "", config)
        root = ET.fromstring(test_xml)
        deadlocks.obfuscate_xml(root)
...
def test_deadlock_xml_bad_format(instance_docker):
    test_xml = """
    <event name="xml_deadlock_report" package="sqlserver" timestamp="2024-08-20T08:30:35.762Z">
     <data name="xml_report">
      <type name="xml" package="package0"/>
    ...
    deadlocks = Deadlocks(check, "", config)
    root = ET.fromstring(test_xml)
    result = deadlocks.obfuscate_xml(root)
